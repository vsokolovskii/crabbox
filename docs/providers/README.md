...
| [e2b](e2b.md) | built-in; `delegated-run` · delegated-sandbox | No SSH; `archive-sync` · direct only; features: `url-bridge`, `run-session` | `linux`; E2B sandbox | `provider-managed`; GPU: no | E2B; sandbox kill or expiry | Hosted Linux sandbox through delegated file/process APIs | No normal SSH lease; envd and preview URLs use E2B hostnames |
